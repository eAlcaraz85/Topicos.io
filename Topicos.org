...
Django incluye un servidor de desarrollo ligero, ideal para pruebas rápidas durante el desarrollo.

- **Características**:
  - Es simple y fácil de usar.
  - Reinicia automáticamente al detectar cambios en el código.
  - No está diseñado para producción, ya que no es seguro ni optimizado para manejo de alta concurrencia.
...
- **Uso**:
  ...
  Por defecto, el servidor escucha en `http://127.0.0.1:8000`, pero puedes cambiar el puerto o la dirección IP:
  #+begin_src shell
  python manage.py runserver 0.0.0.0:8080
  #+end_src

** Servidor para producción
En producción, el servidor de desarrollo no es adecuado. Django utiliza servidores más robustos como WSGI o ASGI para manejar aplicaciones en un entorno de producción.

*** Servidores WSGI
- **Descripción**: WSGI (Web Server Gateway Interface) es el estándar para aplicaciones web en Python.
- **Opciones populares**:
...
*** Servidores ASGI
- **Descripción**: ASGI (Asynchronous Server Gateway Interface) es una evolución de WSGI que permite manejar solicitudes asincrónicas, como WebSockets.
- **Opciones populares**:
...
En un entorno de producción, Django suele combinarse con un servidor web para manejar las solicitudes HTTP y servir archivos estáticos.
...
** Qué es el MVT
El patrón MVT (Modelo-Vista-Template) es una variación del patrón MVC (Modelo-Vista-Controlador). En Django:
- **Modelo (Model)**: Gestiona la base de datos y la lógica de acceso a los datos.
- **Vista (View)**: Gestiona la lógica de negocio y responde a las solicitudes del usuario.
...
** Vista (View)
Crea una vista en `views.py` que obtenga datos del modelo y los pase a un template:
   #+begin_src python
...
** Template
Crea un archivo HTML para mostrar los datos en `templates/mi_aplicacion/lista_articulos.html`:
   #+begin_src html
   <!DOCTYPE html>
   <html>
   <head>
