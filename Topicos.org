...
* Servidor en Django
Django utiliza diferentes tipos de servidores según el entorno de
desarrollo o producción. 

** Servidor de desarrollo
Django incluye un servidor de desarrollo ligero, ideal para pruebas
rápidas durante el desarrollo.

- **Características**:
  - Es simple y fácil de usar.
  - Reinicia automáticamente al detectar cambios en el código.
  - No está diseñado para producción, ya que no es seguro ni
    optimizado para manejo de alta concurrencia.
...
- **Uso**:
  ...
  Por defecto, el servidor escucha en `http://127.0.0.1:8000`, pero
  puedes cambiar el puerto o la dirección IP:
  #+begin_src shell
  python manage.py runserver 0.0.0.0:8080
  #+end_src

** Servidor para producción
En producción, el servidor de desarrollo no es adecuado. Django
utiliza servidores más robustos como WSGI o ASGI para manejar
aplicaciones en un entorno de producción.

*** Servidores WSGI
- **Descripción**: WSGI (Web Server Gateway Interface) es el estándar
  para aplicaciones web en Python.
- **Opciones populares**:
  - **Gunicorn**: Rápido, minimalista y fácil de configurar.
  - **uWSGI**: Altamente configurable y con soporte para múltiples protocolos.
...
*** Servidores ASGI
- **Descripción**: ASGI (Asynchronous Server Gateway Interface) es una
  evolución de WSGI que permite manejar solicitudes asincrónicas, como
  WebSockets.
- **Opciones populares**:
  - **Daphne**: Ideal para aplicaciones Django con soporte para WebSockets.
  - **Hypercorn**: Compatible con HTTP/2 y asincronía.

- **Ejemplo de uso con Daphne**:
  #+begin_src shell
  daphne mi_proyecto.asgi:application
  #+end_src

** Configuración típica para producción
En un entorno de producción, Django suele combinarse con un servidor
web para manejar las solicitudes HTTP y servir archivos estáticos.

*** Servidor web frontend
- **Opciones comunes**:
  - **Nginx**: Popular como proxy inverso y servidor de archivos estáticos.
  - **Apache**: Compatible con mod_wsgi para integrar aplicaciones WSGI.

*** Configuración recomendada
- Nginx como servidor web frontend.
- Gunicorn o uWSGI como servidor de aplicaciones backend.
- Configuración adicional para servir archivos estáticos y de medios.

** Resumen
- El servidor de desarrollo (`runserver`) es ideal para pruebas y desarrollo local.
- En producción, se recomienda usar servidores WSGI/ASGI (como Gunicorn, uWSGI o Daphne) en combinación con Nginx o Apache para garantizar seguridad, rendimiento y escalabilidad.


* Modelo-Vista-Template (MVT) en Django

** Qué es el MVT
El patrón MVT (Modelo-Vista-Template) es una variación del patrón MVC
(Modelo-Vista-Controlador). En Django:
- **Modelo (Model)**: Gestiona la base de datos y la lógica de acceso a los datos.
- **Vista (View)**: Gestiona la lógica de negocio y responde a las
  solicitudes del usuario.
- **Template (Plantilla)**: Define cómo se muestra la información al usuario.

** Crear un proyecto Django
1. Instalar Django si aún no lo tienes:
   #+begin_src shell
   pip install django
   #+end_src
...
1. Registrar la nueva aplicación en `settings.py`:
   #+begin_src python
   INSTALLED_APPS = [
...
** Vista (View)
Crea una vista en `views.py` que obtenga datos del modelo y los pase a
un template:
   #+begin_src python
...
** Template
Crea un archivo HTML para mostrar los datos en
`templates/mi_aplicacion/lista_articulos.html`:
   #+begin_src html
   <!DOCTYPE html>
   <html>
   <head>
   ...
   <body>
       ...
       <ul>
           {% for articulo in articulos %}
               <li>{{ articulo.titulo }} - Publicado el {{ articulo.fecha_publicacion }}</li>
           {% endfor %}
       </ul>
   </body>
   </html>
...
Añade una URL para la vista en `urls.py` de tu aplicación:
   #+begin_src python
...
Incluye las URLs de la aplicación en `urls.py` del proyecto:
   #+begin_src python
...
2. Abre tu navegador y ve a `http://127.0.0.1:8000/articulos/`.
bootstrap
** Configuración con postrgres
#+BEGIN_SRC sql
DATABASES = {
    'default': {
...
** Resumen
- **Modelo**: Define y gestiona los datos.
- **Vista**: Procesa las solicitudes y pasa datos al template.
- **Template**: Renderiza los datos y los muestra al usuario.

laravel modelo vista controlador 



* Código de Flet a Django
** Requisitos Previos
...
- Configurar PostgreSQL:
  ...
  - Configurar un usuario con acceso.

** Crear el Proyecto Django
1. Crear un nuevo proyecto:
   #+begin_src bash
   django-admin startproject gestion_personas
   cd gestion_personas
   #+end_src
...
*** Configuración de la Base de Datos en =settings.py=
   ...
   DATABASES = {
       'default': {
           'ENGINE': 'django.db.backends.postgresql',
           'NAME': 'ejemplo',  # Nombre de tu base de datos
           'USER': 'postgres',
...
Edita el archivo =models.py= en la aplicación =personas=:
   #+begin_src python
   from django.db import models
...
** Crear la Vista y Configurar URLs
*** Definir las Vistas
Edita el archivo =views.py= en la aplicación =personas=:
   #+begin_src python
...
*** Configurar las URLs
Edita el archivo =urls.py= en la aplicación =personas=:
   #+begin_src python
   from django.urls import path
   from . import views
...
En el archivo =urls.py= principal del proyecto, incluye las URLs de la aplicación:
   #+begin_src python
   from django.contrib import admin
   from django.urls import path, include
...
Crea una carpeta =templates= dentro de la aplicación =personas= y agrega los siguientes archivos:

*** Plantilla: =lista_personas.html=
   #+begin_src html
   <!DOCTYPE html>
   <html>
   <head>
   ...
   <body>
       ...
       <ul>
           {% for persona in personas %}
               <li>
                   ID: {{ persona.id }}, Nombre: {{ persona.nombre }}, Edad: {{ persona.edad }}
                   <a href="/modificar/{{ persona.id }}/">Modificar</a>
                   <a href="/eliminar/{{ persona.id }}/">Eliminar</a>
               </li>
           {% endfor %}
       </ul>
   </body>
...
*** Plantilla: =agregar_persona.html=
   #+begin_src html
   <!DOCTYPE html>
   <html>
   <head>
...
*** Plantilla: =modificar_persona.html=
   #+begin_src html
   <!DOCTYPE html>
   <html>
   <head>
...
2. Abre tu navegador en [[http://127.0.0.1:8000/][http://127.0.0.1:8000/]].

** ¡Listo!
Con esto, tendrás una aplicación funcional para la gestión de personas utilizando Django y PostgreSQL.



* Lectura de datos de un sensor con Arduino y Raspberry Pi
Este apartado proporciona una guía para leer datos de un sensor
utilizando Arduino, enviarlos a una Raspberry Pi y visualizarlos en
una interfaz web desarrollada con Django.

** Hardware y software necesarios
- **Hardware**:
  - Arduino Uno o similar.
  - Raspberry Pi (cualquier modelo compatible).
  - Sensor (por ejemplo, DHT11 o DHT22 para temperatura y humedad).
  - Cables y protoboard.

- **Software**:
  - Arduino IDE.
  - Python 3 en Raspberry Pi.
  - Django (framework web).
  - Biblioteca para el sensor (instalada en Arduino y/o Raspberry Pi).

** Paso 1: Configurar Arduino para leer el sensor
1. Conecta el sensor al Arduino según el esquema del fabricante.
2. Escribe el código para leer el sensor y enviar los datos por el puerto serie:
   #+begin_src cpp
   #include <DHT.h>
   ...
   #+end_src
3. Sube este código al Arduino usando el Arduino IDE.

** Paso 2: Configurar Raspberry Pi para recibir datos
1. Instala las bibliotecas necesarias en la Raspberry Pi:
   #+begin_src shell
   sudo apt update
...
2. Crea un script para leer los datos enviados por el Arduino:
   #+begin_src python
   import serial

   def leer_datos_serial():
       puerto = serial.Serial('/dev/ttyUSB0', 9600)  # Ajusta el puerto según corresponda
       while True:
...
** Paso 3: Configurar Django para mostrar los datos
1. Crea un nuevo proyecto Django:
   #+begin_src shell
   django-admin startproject proyecto_sensor
   cd proyecto_sensor
   python manage.py startapp sensor
   #+end_src

2. Crea un modelo en `sensor/models.py` para almacenar los datos del sensor:
   #+begin_src python
   from django.db import models
   ...
   class LecturaSensor(models.Model):
       ...
       def __str__(self):
           return f"{self.fecha_hora}: {self.temperatura}°C, {self.humedad}%"
   #+end_src
...
4. Crea una vista en `sensor/views.py` para mostrar las lecturas:
   #+begin_src python
   from django.shortcuts import render
   from .models import LecturaSensor

   def mostrar_lecturas(request):
       lecturas = LecturaSensor.objects.all().order_by('-fecha_hora')[:10]
       return render(request, 'sensor/lecturas.html', {'lecturas': lecturas})
   #+end_src

5. Configura las URLs en `sensor/urls.py`:
   #+begin_src python
   from django.urls import path
   from . import views

   urlpatterns = [
       path('', views.mostrar_lecturas, name='mostrar_lecturas'),
   ]
   #+end_src

   Y registra las URLs en `proyecto_sensor/urls.py`:
   #+begin_src python
   from django.urls import path, include
...
6. Crea una plantilla HTML en `templates/sensor/lecturas.html`:
   #+begin_src html
   <!DOCTYPE html>
   <html>
   <head>
...
** Paso 4: Integrar Raspberry Pi con Django
Crea un script para guardar las lecturas del sensor en la base de datos Django:
   #+begin_src python
   import serial
   import requests

   URL = "http://127.0.0.1:8000/api/guardar_lectura/"  # Cambia según tu configuración
   ...
   def enviar_datos():
       ...
       while True:
           ...
           if "Temperatura" in datos and "Humedad" in datos:
               temperatura, humedad = [float(x.split(": ")[1]) for x in datos.split("  ")]
               response = requests.post(URL, data={'temperatura': temperatura, 'humedad': humedad})
...
2. Ejecuta el script de la Raspberry Pi para enviar datos.

3. Abre el navegador y ve a `http://127.0.0.1:8000` para ver las lecturas.


* Proyecto Completo Arduino
** Configuración inicial del proyecto
*** Crear el proyecto y la aplicación
#+BEGIN_SRC bash
django-admin startproject gestion_sensores
cd gestion_sensores
python manage.py startapp sensores
#+END_SRC

*** Configuración de aplicaciones en settings.py
Edite el archivo `gestion_sensores/settings.py` para agregar la aplicación.
...
Usaremos SQLite por simplicidad, que ya está configurada por defecto.

---

** Modelos
Cree el archivo `sensores/models.py` para definir los modelos de usuario y datos del sensor.
...
class SensorData(models.Model):
    ...
    def __str__(self):
        return f"{self.usuario.username} - {self.valor} - {self.fecha_hora}"
#+END_SRC

---

** Formularios
Cree un archivo `sensores/forms.py` para manejar los formularios de entrada.
...
** Vistas
Implemente las vistas en `sensores/views.py` para manejar las operaciones de login, altas, bajas, modificaciones y consultas.
...
# Listar datos del sensor
@login_required
def listar_datos(request):
    datos = SensorData.objects.filter(usuario=request.user)
    return render(request, 'listar_datos.html', {'datos': datos})

# Crear dato del sensor
@login_required
def crear_dato(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            sensor_data.usuario = request.user
            sensor_data.save()
            return redirect('listar_datos')
    else:
        form = SensorDataForm()
    return render(request, 'crear_dato.html', {'form': form})

# Editar dato del sensor
@login_required
def editar_dato(request, id):
    dato = get_object_or_404(SensorData, id=id, usuario=request.user)
    if request.method == 'POST':
        form = SensorDataForm(request.POST, instance=dato)
        if form.is_valid():
            form.save()
            return redirect('listar_datos')
    else:
        form = SensorDataForm(instance=dato)
    return render(request, 'editar_dato.html', {'form': form})

# Eliminar dato del sensor
@login_required
...
** URLs
Defina las rutas en `sensores/urls.py`.
...
urlpatterns = [
    ...
    path('', views.listar_datos, name='listar_datos'),
    path('crear/', views.crear_dato, name='crear_dato'),
    path('editar/<int:id>/', views.editar_dato, name='editar_dato'),
    path('eliminar/<int:id>/', views.eliminar_dato, name='eliminar_dato'),
]
#+END_SRC

Configure el archivo principal de URLs en `gestion_sensores/urls.py`.
...
** Plantillas HTML
Cree una carpeta `templates/sensores/` y añada las plantillas necesarias.
...
<a href="{% url 'crear_dato' %}">Agregar Dato</a>
<table>
    <tr>
        <th>Fecha</th>
        <th>Valor</th>
    ...
    <tr>
        ...
        <td>{{ dato.valor }}</td>
        <td>
            <a href="{% url 'editar_dato' dato.id %}">Editar</a>
            <a href="{% url 'eliminar_dato' dato.id %}">Eliminar</a>
        </td>
    </tr>
    {% endfor %}
</table>
...
*** `crear_dato.html` y `editar_dato.html`
#+BEGIN_SRC html
<h2>{% if form.instance.id %}Editar Dato{% else %}Crear Dato{% endif %}</h2>
<form method="post">
    {% csrf_token %}
    {{ form.as_p }}
...
Para enviar datos desde Arduino, puedes usar una biblioteca como `WiFi.h` o `Ethernet.h` para enviar datos mediante HTTP POST a tu API de Django.
...
void loop() {
  if (WiFi.status() == WL_CONNECTED) {
    HTTPClient http;
    http.begin("http://tu_dominio.com/crear/"); // Cambia por tu URL
    http.addHeader("Content-Type", "application/json");

    float sensorValue = analogRead(A0);
    String postData = "{\"valor\":" + String(sensorValue) + "}";
...
Accede a `/login/` para comenzar a usar el sistema.


* Integrando Bootstrap en un Proyecto de Django

** Introducción
Bootstrap es una biblioteca de front-end que facilita el diseño de
interfaces web responsivas y modernas. Este apartado muestra cómo
integrarlo en un proyecto de Django.

** Prerrequisitos
- Tener instalado Django en tu entorno.
- Tener un proyecto de Django creado.
...
3. Instala Django si aún no lo has hecho:
   #+BEGIN_SRC shell
   pip install django
   #+END_SRC

*** Crear un proyecto y una aplicación de Django
...
3. Agrega la aplicación al archivo `settings.py`:
   #+BEGIN_SRC python
   INSTALLED_APPS = [
       ...
       'mi_app',
...
*** Integrar Bootstrap en Django
1. Descarga Bootstrap:
   - Visita [[https://getbootstrap.com/]] y copia el enlace al CSS y JS.

2. Crea un directorio para archivos estáticos en tu aplicación:
   #+BEGIN_SRC shell
   mkdir mi_app/static
   mkdir mi_app/static/css mi_app/static/js mi_app/static/img
   #+END_SRC

3. Agrega los enlaces de Bootstrap al archivo `base.html` (archivo de plantilla base):
...
4. Configura `settings.py` para buscar archivos estáticos:
   #+BEGIN_SRC python
   STATIC_URL = '/static/'
   STATICFILES_DIRS = [
...
5. Crea una plantilla heredada para una página ejemplo:
   - En `mi_app/templates/mi_app/index.html`:
     #+BEGIN_SRC html
     {% extends 'base.html' %}
     {% block title %}Inicio{% endblock %}
     {% block content %}
     <h1 class="text-center">Bienvenido a mi Proyecto con Bootstrap</h1>
     <button class="btn btn-primary">Haz clic aquí</button>
     {% endblock %}
     #+END_SRC

*** Probar el proyecto
1. Modifica `urls.py` en la aplicación para mostrar la página de inicio:
   - En `mi_app/urls.py`:
     #+BEGIN_SRC python
     from django.urls import path
     from . import views
...
2. Crea la vista `index` en `mi_app/views.py`:
   #+BEGIN_SRC python
   from django.shortcuts import render
...
3. Incluye las rutas de la aplicación en `mi_proyecto/urls.py`:
   #+BEGIN_SRC python
   from django.contrib import admin
   from django.urls import path, include
...
5. Visita [[http://127.0.0.1:8000/]] para ver la página inicial con Bootstrap integrado.


* Código de Flet a Django con Bootstrap
** Requisitos Previos
- **Python** y **Django** instalados:
  - Instalar Django: ~pip install django~
  - Instalar psycopg2: ~pip install psycopg2~
...
1. Agregar enlaces de Bootstrap a las plantillas HTML:
   - En cada plantilla que crees, incluye lo siguiente en el `<head>` y al final del `<body>`:
...
2. Organizar plantillas con una base:
     ...
     <body>
         <div class="container">
             <nav class="navbar navbar-expand-lg navbar-light bg-light">
                 <a class="navbar-brand" href="/">Gestión Personas</a>
...
** Crear las Plantillas HTML con Bootstrap
Crea una carpeta =templates= dentro de la aplicación =personas= y agrega los siguientes archivos:
...
*** Plantilla: =lista_personas.html=
   ...
   {% block content %}
   <h1 class="text-center mt-4">Lista de Personas</h1>
   <a href="/agregar/" class="btn btn-primary mb-3">Agregar Persona</a>
   <ul class="list-group">
       {% for persona in personas %}
           <li class="list-group-item d-flex justify-content-between align-items-center">
               ID: {{ persona.id }} - Nombre: {{ persona.nombre }} - Edad: {{ persona.edad }}
               <span>
                   <a href="/modificar/{{ persona.id }}/" class="btn btn-warning btn-sm">Modificar</a>
                   <a href="/eliminar/{{ persona.id }}/" class="btn btn-danger btn-sm">Eliminar</a>
               </span>
           </li>
       {% endfor %}
   </ul>
...
*** Plantilla: =agregar_persona.html=
   ...
   {% block content %}
   <h1 class="text-center mt-4">Agregar Persona</h1>
...
*** Plantilla: =modificar_persona.html=
   ...
   {% block content %}
   <h1 class="text-center mt-4">Modificar Persona</h1>
   <form method="POST" class="mt-3">
       {% csrf_token %}
       <div class="mb-3">
           <label for="nombre" class="form-label">Nombre</label>
